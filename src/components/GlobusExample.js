// Source this script: https://unpkg.com/@globus/sdk@3.4.0/umd/globus.production.js
...
const manager = globus.authorization.create({
    // Your registered Globus Application client ID. @see https://docs.globus.org/api/auth/developer-guide/#developing-apps
    ...
    // Known scopes required by your application. This example will requires a Transfer scope based on it's usage of `ls`.
...
/** Since **this** page was configured as the `redirect` handler, we will handle
    the code exchange here. This method will automatically no-op if the OAuth `?code=` parameter is not present.*/
...
if (manager.authenticated) {
    ...
    globus.transfer.fileOperations.ls(COLLECTION, {
        headers: {
    ...
    .then((json) => {
        ...
        */
        ...
        if (isError) {
            /** If the error is well-formed, the SDK will be able to provide default handling. We'll avoid the
                handler executing automatically and instead bind it to the `onclick` event of a new `<button>`.*/
            const handler = manager.handleErrorResponse(json, false);
            // If there was an identified error, with a handler, render a button that will call the handler on click.
